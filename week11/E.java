...
public class E {
    ...
    private static int[] weightPerPackage;
    ...
    private static void doStuff() throws IOException {
        ...
        for (int currentArticle = 1; currentArticle <= n; currentArticle++) {
            ...
            for(int currentMaxWeight = 1; currentMaxWeight <= w; currentMaxWeight++){
                maxCalories[currentArticle][currentMaxWeight] = 0;
                for(int k = 0; k <= available; k++){
                    if(k * articleWeight > currentMaxWeight){
                        break;
                    }
 
                    if(maxCalories[currentArticle][currentMaxWeight] < k * calories + maxCalories[currentArticle - 1][currentMaxWeight - k * articleWeight]){
                        maxCalories[currentArticle][currentMaxWeight] = k * calories + maxCalories[currentArticle - 1][currentMaxWeight - k * articleWeight];
                        itemsTaken[currentArticle][currentMaxWeight] = k;
                    }
                }
